 ...
 const SearchBar=()=>{
      ...
      <form onSubmit={handleSubmit} className="-mt-8 p-3 bg-orange-400 rounded shadow-md grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 items-center gap-4">
           ...
           <input className="w-full p-1 focus:outline-none font-bold" type="number" value={adultCount} min={1} max={20} onChange={(event)=>setAdultCount(parseInt(event.target.value))} />
           </label>
           <label className="items-center flex">Children:
           <input className="w-full p-1 focus:outline-none font-bold" type="number" value={childCount} min={0} max={6} onChange={(event)=>setChildCount(parseInt(event.target.value))} />
           </label>
           </div>
           <div>
           <DatePicker
      ...
      <div className="flex gap-1">
       <button
      type="submit" // Ensures the form submits on click
